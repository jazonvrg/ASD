class enable_full_tx_fifo_test extends uart_base_test;
	`uvm_component_utils(enable_full_tx_fifo_test)
	...
	function new(string name = "enable_full_tx_fifo_test", uvm_component parent);
	...
	virtual task run_phase(uvm_phase phase);
		...
		$display("============================================================================================================================");
		$display("============================================  ### FULL TX FIFO | ENABLE  ###  ==============================================");
		$display("============================================================================================================================");
		if (cfg.randomize() with {uart_mode == TX;}) begin
	...
	virtual task run_process();
		calc_divisor(cfg);
		regmodel.IER.write(status, {27'h0, 1'b0, 1'b0, 1'b0, 1'b0, 1'b1});
		if (cfg.ovsmpl == uart_configuration::X16) begin
		...
		regmodel.FSR.read(status, rdata);
		env.scb.compare(ahb_vif.interrupt, 1'b1);
		// full_status = 0
		env.scb.selection = 1.1;
		if (cfg.parity_mode != uart_configuration::NONE) begin
