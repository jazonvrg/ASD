class TX_13x_odd_parity_test extends uart_base_test;
	`uvm_component_utils(TX_13x_odd_parity_test)
	...
	function new(string name = "TX_13x_odd_parity_test", uvm_component parent);
			...
			foreach(stp_bit[i_stp]) begin
				foreach(prt_error[i_error]) begin
					...
					foreach(bd_rate[i_bd]) begin
						if (cfg.randomize() with {parity_mode == ODD; 
